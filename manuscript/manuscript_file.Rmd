...
The emission rates reported here span a broad range (`r min.max$min %>% round(.,2)` - `r min.max$max %>% round(.,2)` mg N~2~O m^-2^ d^-1^), which was expected for this large and diverse collection of waterbodies, but are within the range of values reported in the literature for lentic ecosystems [@Yang_etal_2015; @Kortelainen_etal_2020; @Webb_etal_2019]. Variability and magnitude of predicted N~2~O emission rates decreased with increasing waterbody size **(Fig. 5A)**, which is consistent with the relationship between N~2~O saturation and waterbody size **(Fig. 3C,D)**. Despite predicted widespread N~2~O undersaturation in CONUS waterbodies, the estimated national average N~2~O emission rate was positive (mean = `r national_stats$post_med_en2o`, 95% CI: `r national_stats$LCL_en2o` - `r national_stats$UCL_en2o` mg N~2~O m^-2^ d^-1^) **(Table 1)**. This contradiction is due to relatively rare waterbodies with very high emission rates that pull the mean (`r national_stats$post_med_en2o` mg N~2~O m^-2^ d^-1^) to a value that far exceeded the median (`r national_stats$post_median_en2o` mg N~2~O m^-2^ d^-1^) **(SI Fig. 1)**.

![Figure 5: A) N~2~O emission rate and B) flux plotted against waterbody size for all 465,896 waterbodies in the target population. Across all plots, points and bars reflect the mean (points) and 95% credible interval, respectively, of the 2000 realizations of the posterior predictive distribution.](manuscript_figures/n2oFluxAndEmissionRateVsContinuousArea.tiff)

## Fluxes

Total N~2~O flux from the surface of CONUS waterbodies is a function of N~2~O emission rates and waterbody surface areas. The four smallest size classes (\< 50 Ha) in the survey design represented `r round(proportionSmallNumber, 2) * 100`% of all lakes in the population but were estimated to have contributed only `r round((FluxBySize %>% filter(size_cat != "50_max") %>% select(estimate) %>% summarise(tot_small = sum(estimate)) %>% pull())/(FluxBySize %>% filter(size_cat == "50_max") %>% select(estimate) %>% pull()) * 100, 0)`% to the N~2~O flux from CONUS waterbodies. The reason for this pattern is two-fold. First, lakes in the smaller size classes constituted only `r round(proportionSmallSA, 2) * 100`% of total waterbody surface area, thereby limiting their N~2~O flux. Second, widespread N~2~O uptake in these size classes largely offset the very high emission rates predicted to occur in a minority of the population. Waterbodies in the largest size class (\> 50 Ha) represented only `r round(1 - proportionSmallNumber, 2) * 100`% of the total number of lakes in the population but had `r round(1-proportionSmallSA, 2) * 100`% of total lake surface area. This lake size class was the source of `r ((FluxBySize %>% filter(size_cat == "50_max") %>% select(estimate) %>% pull()) / (national_stats %>% select(post_med_fn2o) %>% pull()) * 100) %>% round(., 0)`% of the total N~2~O flux (mean = `r FluxBySize %>% filter(size_cat == "50_max") %>% select(estimate) %>% pull() %>% round(0)` metric tons N~2~O year^-1^), but the flux estimate is poorly constrained with a 95% credible interval of `r FluxBySize %>% filter(size_cat == "50_max") %>% select(LCL) %>% pull()` - `r FluxBySize %>% filter(size_cat == "50_max") %>% select(UCL) %>% pull()` metric tons N~2~O year^-1^. While uncertainty in N~2~O emission rates was smallest for large lakes **(Fig. 6A)**, these small uncertainties propagated to large uncertainties in flux when expressed across the surface area of large waterbodies **(Fig. 6B)**.
...
The mean flux estimate for CONUS waterbodies was positive (`r national_stats %>% select(post_med_fn2o) %>% pull() %>% round(., 0)` metric tons N~2~O year^-1^); however, the credible interval was quite broad and included zero (`r national_stats %>% select(LCL_fn2o) %>% pull() %>% round(., 0)` - `r national_stats %>% select(UCL_fn2o) %>% pull() %>% round(., 0)` metric tons N~2~O year^-1^) so we could not conclude with statistical confidence whether CONUS waterbodies were a net source or sink of N~2~O overall. Though we are unaware of independent estimates of N~2~O emissions from CONUS lakes and reservoirs, the Intergovernmental Panel on Climate Change (IPCC) estimated that CONUS drainage networks, inclusive of zero order streams through major river mouths, emit `r format(ipcc.n2o.surface, scientific = FALSE, big.mark = ",")` metric tons N~2~O year^-1^ [@epa_2023]. The IPCC estimates total N~2~O emissions from aquatic ecosystems as a fraction of the anthropogenic nitrogen that leaches from soils into surface water systems but does not distinguish standing from flowing waters, thus a direct comparison between the IPCC estimate and the value reported here is not possible; however, the mean N~2~O  flux from lakes and reservoirs reported here comprises only `r round((national_stats %>% select(post_med_fn2o) %>% pull() / ipcc.n2o.surface) * 100, digits = 1)` % of the IPCC estimate, suggesting the IPCC estimate is too high, possibly because the IPCC model does not allow for N~2~O consumption in lakes, ponds, and reservoirs. This limitation extends to many mechanistic models used to estimate N~2~O fluxes from inland waters [@Lauerwald_etal_2019;@Marzadri_etal_2021;@Marzadri_etal_2017].  Given our estimate of widespread N~2~O consumption in CONUS waterbodies, failure to include this dynamic in aquatic N~2~O models may yield low-biased emission estimates [@Lauerwald_etal_2023]. Recent model parameterizations that allow for N~2~O consumption in inland waters are promising developments in this regard [@Yao_etal_2019; @Wang_etal_2023;@Maavara_etal_2019]. 
