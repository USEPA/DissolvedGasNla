...
Roy Martin modeled the uncertainty in the N2O saturation ratio in DG_sensitivity_to_measurement_error.Rmd (Saturation ratio/Standard GC sections). The absolute error in the simulated values were written to desk and read in below. These error estimates are used to define uncertainty in the N2O saturation ratio.
...
S_a_me_abs <- readRDS(paste0(localPath, 
                             "/Environmental Protection Agency (EPA)/",
                             "ORD NLA17 Dissolved Gas - Documents/",
                             "inputData/S_a_me_abs.RDS"))
...
dg <- dg %>%
  ...
  mutate(dissolved.ch4.umol = dissolved.ch4 * 1000000, # mol/L -> umol/L
         ...
         # recompute using error from Roy's uncertainty simulations
