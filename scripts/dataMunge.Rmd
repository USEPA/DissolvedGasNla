...
The emission rate is the mass flux of a gas into or out of a waterbody.  Emission rates are typically expressed as mass m^-2^ day^-1^.  The phrase 'emission rate' is used interchangeably with 'Flux rate' and is often reported in the literature as (F~gas~), where the subscript 'gas' refers to the gas of interest (i.e. N~2~O, CO~2~, CH~4~).  Emission rate can be  calculated as the product of gas^\*^ and the gas transfer velocity (k):
...
dg <- dg %>% 
  mutate(
    # f.ch4 umol CH4/m2/d 
    f.ch4 = ((dissolved.ch4 - (sat.ch4 * 1000000))*1000) * # 1000000 mol -> umol.  1000 L to m3
           (k600.day * (24/100)), # 24 hour to day.  100 cm to m
    # f.co2 umol CO2/m2/d
    f.co2 = ((dissolved.co2 - (sat.co2 * 1000000))*1000) * # 1000000 mol -> umol.  1000 L to m3
           (k600.day * (24/100)), # 24 hour to day.  100 cm to m
        # f.n2o nmol N2O/m2/d
    f.n2o = ((dissolved.n2o - (sat.n2o * 1000000000))*1000) * # 1000000 mol -> nmol.  1000 L to m3
           (k600.day * (24/100)), # 24 hour to day.  100 cm to m
    # total flux of CH4 per day kmol CH4 day-1
    f.ch4.d = f.ch4 * area.ha * 10000 * (1/10^9), # 1ha = 10,000 m2. 10^9 umol->mmol->mol->kmol
    # total flux of CO2 per day kmol CO2 day-1
    f.co2.d = f.co2 * area.ha * 10000 * (1/10^9), # 1ha = 10,000 m2. 10^9 umol->mmol->mol->kmol
    # total flux of N2O per day mol N2O day-1
    f.n2o.d = f.n2o * area.ha * 10000 * (1/10^9)) %>% # 1ha = 10,000 m2. 10^9 nmol->umol->mmol->mol
         filter(state != "AK", sample.source == "DG")
...
cdf.dg <- left_join(cdf, 
                    select(dg.1, site.id, visit.no, f.ch4, f.co2, 
                           f.n2o, f.ch4.d, f.co2.d, f.n2o.d) %>%
