...
objs <- c("dg",
          ...
          #"all_predictions_b",
          "n2o_survey_ests",
          "n2o_survey_ests_size",
          "sat_survey_ests",
          "sat_survey_ests_size"
          )
...
f <- list()
for(i in 1:N){
    f[i] <- paste0(Sys.getenv("RENV_PROJECT"), "/modelFiles/",
    objs[i], ".rda")
    }
...
The first model for the two N2O responses, conditioned only on the study design factors.
...
```{r conditional_effects_mod_n2osat_temp_area_F, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
effect_no3_surftemp <- expand.grid(no3_cat = levels(df_model$no3_cat),
                               surftemp = round(quantile(df_model$surftemp, 
                                                   probs = seq(0.10, 0.90, 0.2)), 0),
                               log_area = mean(df_model$log_area),
