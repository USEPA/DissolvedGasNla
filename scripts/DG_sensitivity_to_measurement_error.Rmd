...
```{r c_sat}
C_sat <- C_a * B * 1e-6 * (H  * exp(2700*(1/(T_c + 273.15) - 1/298.15))) 
...
```{r simulate_me_sat_C_a, message=F}
C_me_sat <- rep( NA, 1e4 )
...
for( i in 1:length( C_me_sat ) ){
    C_me_sat[i] <- C_a_me[i] * B * 1e-6 * (H  * exp(2700*(1/(T_c + 273.15) - 1/298.15)))
}

# combine into dataframe for ggplot. qplot, which accepts vectors,
# was deprecated in ggplot 3.4.0
C_sat_df <- tibble(C_me_sat = C_me_sat, C_a_me = C_a_me)

ggplot(C_sat_df, aes(x = C_a_me )) +  
  geom_histogram( binwidth = 0.0001 ) + 
  geom_vline( xintercept = C_a , color = 'red' ) + 
  ggtitle( "Simulated measurement error in air gas concentration") +
...
for( i in 1:length( C_me_sat ) ){
    C_me_sat[i] <- C_a * B * 1e-6 * (H  * exp(2700*(1/(T_c_me[ i ] + 273.15) - 1/298.15)))
}
...
for( i in 1:length( C_me_sat ) ){
    C_me_sat[i] <- C_a * B_me[ i ] * 1e-6 * (H  * exp(2700*(1/(T_c + 273.15) - 1/298.15)))
}
...
for( i in 1:length( C_me_sat ) ){
    C_me_sat[i] <- C_a_me[ i ] * B_me[ i ] * 1e-6 * (H  * exp(2700*(1/(T_c_me[ i ] + 273.15) - 1/298.15)))
}
...
C_ratio <- (1e-6 * B * ( V_g * ( C_e - C_a ) / ( 8.3144598 * ( T_c + 273.15 ) * V_w ) + H * exp( 2700 * ( 1 / ( T_c + 273.15 ) - 1 / 298.15 ) ) * C_e )) /
  (C_a * B * 1e-6 * (H  * exp(2700*(1/(T_c + 273.15) - 1/298.15)))) 
...
C_me_ratio[ i ] <- (1e-6 * B_me[i] * ( V_g_me[i] * ( C_e_me[i] - C_a_me[i] ) / ( 8.3144598 * ( T_c_me[i] + 273.15 ) * V_w_me[i] ) + H * exp( 2700 * ( 1 / ( T_c_me[i] + 273.15 ) - 1 / 298.15 ) ) * C_e_me[i] )) /
  (C_a_me[i] * B_me[i] * 1e-6 * (H  * exp(2700*(1/(T_c_me[i] + 273.15) - 1/298.15)))) 
}
...
sd(C_me_ratio) # 0.0.05479

# the 95% CI is therefore the computed value +/- 2 standard deviations
sd(C_me_ratio) * 1.96 # +/- 0.1073

# so any ratio within 0.1073 of 1.0 cannot be discerned as a source or sink?
```
