...
all_predictions <- all_predictions %>%
  mutate(WSA9 = factor(WSA9,
                       ...
                       labels = c("Northern Plains", "Temperate Plains",
                                  "Upper Midwest", "Coastal Plains", "Southern Plains", "Southern Appalachians", "Xeric", "Northern Appalachians", "Western Mountains")))
...
p <- all_predictions %>%
  ...
  ggplot( aes( x = preds_sat, group = .draw, color = .draw ) ) +
...
all_predictions.w <- full_join(all_predictions, wind, by = c(".row" = ".id")) # too much for laptop,30 seconds on VM
dim(wind) # 465897
dim(all_predictions) #232948500, 500 observations for each lake
465897*500 #232948500
dim(all_predictions.w) #wind.sampFr. good
...
national.lakes <- all_predictions.w %>% 
  group_by(.draw) %>% 
...
inventory %>% mutate(source = as.factor(source),
  ...
  theme_bw() +
  theme(axis.title.y = element_blank())
ggsave(filename = "output/figures/n2oInventory.tiff")
...
(national.lakes$Gg/sum(inventory$Gg)) * 1000 #1.6%
```
...
all_predictions.w %>% filter(.draw == 1) %>% ggplot(aes(x=f.n2o.Mg.y)) + geom_density() +geom_rug() + facet_wrap(~WSA9, scales = "free")
...
# summarize means by WSA9 (posterior median for mean and 95% credible intervals)
# this is analogous to spsurvey "population estimates"
ecoreg_stats <- all_predictions.w %>%
  group_by(WSA9, .draw) %>%
  summarise( mean_n2o = mean(preds_n2o),
             mean_n2oeq = mean(preds_n2oeq),
             mean_sat = mean(preds_sat),
