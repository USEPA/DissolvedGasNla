...
# all_predictions.w.small <- all_predictions %>% filter(.draw %in% 1:5)
# 
...
all_predictions %>%
  group_by(WSA9, .draw) %>% # group by iteration
  #group_by(.draw) %>%
  summarise(mean_f = mean(f.n2o.Mg.y)) %>% # 500 means for each WSA9
  # now summarize to 1 statistic per WSA9
...
all_predictions %>%
  group_by(WSA9, size_cat, .draw) %>% # group by iteration
  #group_by(.draw) %>%
  summarise(mean_f.n2o.Mg.y = mean(f.n2o.Mg.y)) %>% # 500 means for each WSA9 x size cat
  # now summarize to 1 statistic per WSA9
...
all_predictions %>% 
  ...
  group_by(WSA9, size_cat2) %>% 
  summarise(total.n2o = sum(f.n2o.Mg.y)) %>% 
  ggplot(aes(x=size_cat2, y=total.n2o)) + 
  geom_bar(stat = "identity") + 
  facet_wrap(~WSA9, scales = "free")
